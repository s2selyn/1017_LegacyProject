...
public interface BoardMapper {
	...
	// 이것이 왜 마음에 안드는가? 이름도 네단어 조합인데 메소드가 기능을 두개하고있음, 하나씩 조회하는게 아니라 두개를 조회해온다
	// 서비스 구현체 보면 하나의 메소드가 하나의 작업만 수행할 수 있게 책임분리 해뒀다
	// 매퍼에서 두개 같이 가져가는게 재활용성 측면에서 별로다, 보드만 가져가야할일/리플만 가져갈일이 생길 수 있으니 따로만들면 재활용 할 수 있을텐데
	// 이렇게 생긴 메소드를 보면 하나의 메소드가 너무 많은 작업을 하고있나? 생각, 재활용성측면에서는 별로, 자원사용적 측면에서는 이게이득일지도, 원래 두번 DB에 가야하는 논리적 물리적으로 멀리있는 곳에 연결시도하니 시간추가되고 레이턴시 발생
	// 연결하는 커넥션 객체 생성, 사용이 자원많이 소모, select 접근, sql문 실행규칙세우기도 많이 자원씀, 그러니 두번아니고 한번으로 줄이면 자원소모 측면에서는 효율적임
	// 팀장성향에 따라 개발방식이 달라질수도, 보편적으로는 재량에 맡긴다
	// SQL문은 신입개발자한테 짜라고 안한다, 기본적으로는 짜진걸 준다 돌려보라고
	// 스타트업 갈거 아니면 기존개발, 솔루션, sql문 등이 있을것이다, 이전에 회사나 팀이 개발했던 방식이 있을것이니 그걸 준다
	// 개발자가 만들지만 다 회사 소유지, 회사코드가 다 있음 -> board-mapper.xml 수정
